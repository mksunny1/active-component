import { ComplexComponentAction } from "./component-action.js";
...
export class ComponentLocalAction extends ComplexComponentAction {
    constructor() {
        super(...arguments);
        this.reactionKeys = new WeakMap();
    }
    static { this.attr = 'loc-al'; }
    ...
    createActions(context) {
        ...
        const { ComplexActionComponent, attr, setPrefix, callPrefix } = this.constructor;
        const result = {};
        if (context.element.hasAttribute(attr)) {
            context.closedElement = true;
            let path = context.element.getAttribute(attr);
            if (!path || !path.startsWith(setPrefix))
                return result;
            ...
            if (localActionComponent.hasOwnProperty('key'))
                this.reactionKeys.set(context.element, localActionComponent.key);
